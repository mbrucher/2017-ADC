/**
 * \file DiodeClipperFilter_v4.cpp
 */

#include "DiodeClipperFilter_v4.h"
...
namespace ATKADC
{
  ...
  class DiodeClipperFilterv4<DataType_>::SimpleOverdriveFunction
  {
  public:
  ...
  public:
    ...
    std::pair<DataType, DataType> operator()(const DataType* ATK_RESTRICT input, DataType* ATK_RESTRICT output, DataType y1)
    {
	  ...
	  DataType expdiode_y0_p = fmath::exp(y0 / vt);
    ...
    {
      auto expy0 = fmath::exp(y0 / vt);
      auto invexpy0 = 1 / fmath::exp(y0 / vt);
      auto sinh = is * (expy0 - invexpy0);
      auto cosh = is * (expy0 + invexpy0);
      return (y0 + A * (2 * x1 - y0) - B * (2 * sinh - y0 / vt * cosh) ) / (B * cosh / vt + 1 + A);
    }
  };
  
  template <typename DataType>
  DiodeClipperFilterv4<DataType>::DiodeClipperFilterv4()
  ...
  template <typename DataType>
  void DiodeClipperFilterv4<DataType>::setup()
  {
    Parent::setup();
  ...
  template <typename DataType>
  void DiodeClipperFilterv4<DataType>::process_impl(std::size_t size) const
  {
...
BOOST_AUTO_TEST_CASE(DiodeClipperFilterv4_const_sin1k)
{
	...
	ATKADC::DiodeClipperFilterv4<double> filter;
